...
public class PreferEstateApiController {
    ...
    public ResponseEntity<List<EstateListResponseDto>> getRecentList(@Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails user, Pageable pageable) {
        List<EstateListResponseDto> estateListResponseDtos = preferEstateService.getPreferEstate(user.getEmail(), Preference.RECENT, pageable);
    ...
    @ApiResponses({@ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = EstateListResponseDto.class)))})
    @GetMapping("list/dip")
    public ResponseEntity<List<EstateListResponseDto>> getDipList(@Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails user, Pageable pageable) {
        List<EstateListResponseDto> estateListResponseDtos = preferEstateService.getPreferEstate(user.getEmail(), Preference.DIP, pageable);
        return ResponseEntity.status(HttpStatus.OK).body(estateListResponseDtos);
    }
    ...
    public ResponseEntity<List<EstateListResponseDto>> getInquireList(@Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails user, Pageable pageable) {
        List<EstateListResponseDto> estateListResponseDtos = preferEstateService.getPreferEstate(user.getEmail(), Preference.INQUIRY, pageable);
        return ResponseEntity.status(HttpStatus.OK).body(estateListResponseDtos);
    }
