...
public class PreferEstateApiController {
    ...
    public ResponseEntity<List<EstateListResponseDto>> getRecentList(@Parameter(hidden = true) @CurrentUser User user, Pageable pageable) {
        List<EstateListResponseDto> estateListResponseDtos = preferEstateService.getPreferEstate(user, Preference.RECENT, pageable);
    ...
    @ApiResponses({@ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = EstateListResponseDto.class)))})
    @GetMapping("list/dip")
    public ResponseEntity<List<EstateListResponseDto>> getDipList(@Parameter(hidden = true) @CurrentUser User user, Pageable pageable) {
        List<EstateListResponseDto> estateListResponseDtos = preferEstateService.getPreferEstate(user, Preference.DIP, pageable);
        return ResponseEntity.status(HttpStatus.OK).body(estateListResponseDtos);
    }
    ...
    public ResponseEntity<List<EstateListResponseDto>> getInquireList(@Parameter(hidden = true) @CurrentUser User user, Pageable pageable) {
        List<EstateListResponseDto> estateListResponseDtos = preferEstateService.getPreferEstate(user, Preference.INQUIRY, pageable);
        return ResponseEntity.status(HttpStatus.OK).body(estateListResponseDtos);
    }
